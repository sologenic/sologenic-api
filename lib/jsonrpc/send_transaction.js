...
module.exports = function(parameters, callback) {
  ...
  if (parameters.recipient_address.indexOf('?dt=') !== -1 && !parameters.recipient_address.match(/\?dt=[1-9]\d*$/)) {
    return callback({ error: 'invalid_recipient_address' })
  }
  if (!new ripple.RippleAPI().isValidAddress(parameters.recipient_address.replace(/\?dt=[1-9]\d*$/, ''))) {
